...
export const graphqlResolver = {
  ...
  signup: async (
    { signupInput: { email, password, confirmPassword, name } },
    req
  ) => {
    const errors = [];
    ...
    if (
      ...
      error.data = [
        { message: "Name should contain 3 - 100 characters", param: "name" },
      ];
      throw error;
    }
    if (
    ...
    if (password !== confirmPassword) {
      ...
      error.data = [
        { message: "Password do not match", param: "confirmPassword" },
      ];
      error.code = 422;
      throw error;
    }
  ...
  getUserDetails: async (input, req) => {
    ...
    const user = await User.findById(otherUser).populate({
      path: "posts",
      options: {
        sort: { createdAt: -1 },
  ...
  comment: async ({ postId, body, detail }, req) => {
    ...
    if (post.creator.toString() === req.userId.toString()) {
      ...
      if (recipientIndex >= 0) {
        io.getIO().emit(`${recievers[recipientIndex]}`, {
          action: "comment",
          notification: {
  ...
  like: async ({ postId }, req) => {
    ...
    if (isLiked > 0) {
      ...
      error.data = [
        { message: "You have already liked this post", param: "like" },
      ];
  ...
  getPost: async ({ postId, notificationId, page }, req) => {
    ...
    if (notificationId != -1) {
      ...
      if (!notification) {
        ...
        error.data = [
          { message: "Notification not found", param: "notification" },
        ];
    ...
    if (!post) {
      const error = new Error(
        "Post you are trying to see has been removed by the author"
      );
      error.statusCode = 404;
      error.data = [
        "Post you are trying to see has been removed by the author",
